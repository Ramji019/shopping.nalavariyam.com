        ...
        $.ajax({
            ...
            success: function(result) {
                for (let i = 0; i < result.length; i++) {
                    ...
                    }else if (value.attr_type == "radio") {
                        option = "";
                        myArray = value.attr_value.split(",");
                        let j = 0;
                        while (j < myArray.length) {
                            option = option + "<label class='checkbox-inline' >&nbsp;&nbsp;<input name='" +
                                name + "' type='radio' value='" + myArray[j] + "' /> &nbsp;" + myArray[j].toString() + "</label>";
                            j++;
                        }
                        $("#attrdiv").append("<div class='col-md-12 mb-4'> <b>"+value.attr_name + "</b>&nbsp;" +option+"</div>");
